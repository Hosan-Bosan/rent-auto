...
class HostelSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        representation['images'] = CarsImageSerializer(instance.images.all(), many=True, context=self.context).data
        representation['comments'] = CommentSerializer(instance.comments.all(), many=True, context=self.context).data
        representation['likes'] = instance.likes.count()

        rates = Rating.objects.filter(cars=instance)
        if not rates:
            ...
            for i in rates:
                sum = sum + i.rating
            representation['rating'] = Decimal(sum) / Decimal(Rating.objects.filter(cars=instance).count())
    ...
    def create(self, validated_data):
        ...
        cars = Cars.objects.create(**validated_data)
        return cars

class CarsImageSerializer(serializers.ModelSerializer):
    class Meta:
        model = CarsImage
        fields = '__all__'
