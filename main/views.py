...
class CarsViewSet(PermissionMixin, viewsets.ModelViewSet):
...
class CarsImageView( generics.ListCreateAPIView):
    queryset = CarsImage.objects.all()
    serializer_class = CarsImageSerializer
...
class LikeViewSet(PermissionMixin, ModelViewSet):
    ...
    def my_likes(self, request, pk=None):
        queryset = self.get_queryset()
        queryset = queryset.filter(author=request.user)
        serializers = LikeSerializer(queryset, many=True, context={'request': request})
        return Response(serializers.data, 200)
...
class FavoriteViewSet(PermissionMixin, ModelViewSet):
    ...
    def my_favorites(self, request, pk=None):
        queryset = self.get_queryset()
        queryset = queryset.filter(author=request.user)
        serializers = FavoriteSerializer(queryset, many=True, context={'request': request})
        return Response(serializers.data, 200)